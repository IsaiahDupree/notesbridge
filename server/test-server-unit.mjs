...
import { NOTES_WIDGET_HTML, WIDGET_MIME } from './lib/widget.js';
...
test('widget HTML uses data-id delegation, not inline onclick', () => {
  assert.equal(NOTES_WIDGET_HTML.includes('onclick='), false);
  assert.ok(NOTES_WIDGET_HTML.includes('data-id='));
  assert.ok(NOTES_WIDGET_HTML.includes("'&#39;'")); // esc() now covers single quotes
  assert.equal(WIDGET_MIME, 'text/html+skybridge');
});
